...
R = abs(order)
meanRe = mean(R[100:])
#autocorrelation of the abs of the order parameter
N_tau = 1000
auto = zeros(N_tau)
for tau in arange(N_tau):
    auto[tau] = sum((R[0:N_time-N_tau-1]-meanRe)*(R[tau:N_time-N_tau-1+tau]-meanRe))/(N_time-N_tau)
...
plt.plot(auto)
plt.xlabel('$t$')
plt.ylabel('autocorrelation of the absolute of the order parameter')
plt.grid(True)
