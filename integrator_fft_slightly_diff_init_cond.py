...
order2  = ones(N_time)+1j*ones(N_time)
phi2    = ones(N_time)
phi_corr2 = ones(N_time)
phase2pi2 = 0
for i in arange(N_time):
    order2[i] = sum(cos(y2[i])+1j*sin(y2[i]))/N_osc
    phi2[i] = cmath.phase(order2[i])
    if (phi2[i]-phi2[i-1])>(pi):
        phase2pi2 = phase2pi2 - 2.*pi   
    if (-phi2[i]+phi2[i-1])>(pi):
        phase2pi2 = phase2pi2 + 2.*pi
    phi_corr2[i] = phi2[i] + phase2pi2

lin2 = phi_corr2[100-1:]
omg_mean2 = 0 
for k in arange(len(lin2)-1)+1:
    omg_mean2 = omg_mean2 + lin2[k]/k

phi_corr_fluct2 = lin2 + sum(lin2)/len(lin2)
...
plt.plot(order2.real,order2.imag, color = 'black')
plt.plot(order.real[0:20],order.imag[0:20])
plt.plot(order.real[N_time-20:N_time-1],order.imag[N_time-20:N_time-1])
plt.figure(2)
plt.plot(abs(order))
plt.plot(abs(order2), color = 'black')
plt.figure(3)
#plt.plot(phi)
plt.plot(phi_corr_fluct)
plt.plot(phi_corr)
plt.plot(phi_corr_fluct2, color = 'black')
plt.plot(phi_corr2, color = 'black')
plt.figure(4)
plt.plot(fft)
plt.figure(5)
plt.plot(omg)
